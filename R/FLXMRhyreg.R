    ...
    z@defineComponent <- function(para) {  # we get para from the first estimation with start values
      ...
      predict <- function(x, type, ...) {
        ...
        if(formula_type_classic == TRUE){
          ...
          # NON-CLASSIC
          if(type == type_cont){
            p <- eval_formula_non(the$formula_non,
                                  the$data,
                                  para$coef[is.element(names(para$coef),
                                                       c(variables_cont,variables_both))])
          }
          if(type == type_dich){
            p <- eval_formula_non(the$formula_non,
                                  the$data,
                                  para$coef[is.element(names(para$coef),
                                                       c(variables_dich,variables_both))]) * para$theta
          }
        }
    ...
    z@fit <- function(x, y, w, component, ...){
      ...
      if(!exists("counter", envir = the)){
        ...
        if(the$counter < the$k){
          ...
          if(isTRUE(formula_type_classic)){
            stv_new <- setNames(c(component$coef, component$sigma, component$theta),
                                c(colnames(x), "sigma", "theta"))
          }else{
            if(is.list(stv)){
              stv_new <- setNames(c(component$coef, component$sigma, component$theta),
                                  c(names(the$stv[[1]]))) # order important, maybe flexiblize?
            }else{
              stv_new <- setNames(c(component$coef, component$sigma, component$theta),
                                  c(names(the$stv))) # order important, maybe flexiblize?
            }
          }
      ...
      z@defineComponent(para = list(coef = fit_mle@coef[!is.element(names(fit_mle@coef),
                                                                    c("sigma", "theta"))],
